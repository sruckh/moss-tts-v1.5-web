...
| 🔶 Breeze TTS 2 engine — third engine (`BreezeBlue/Breeze-TTS-2`) behind `BREEZE_RUNPOD_ENDPOINT`, with **clone** (reference + transcript), **design** (a written instruction alone; the only job shape that stores `voice_id` NULL), and **direction** (clone + steering instruction) modes; `cfg_scale` support; requests pin `response_delivery: "base64"`; word timings recovered through the whisper sidecar, fail-open | Built; live verification pending deploy |
...
The complete product works end to end: apply, get approved, paste a script, pick or clone a voice, render on MOSS-TTS v1.5 or Higgs TTS, and download the WAV — with accounts, access control, the queue, submission, polling, alignment capture, transcription, and storage all owned by the app. (Breeze TTS 2 lands with the next deploy; see the table above.)
...
internal/jobs/       jobs table: enqueue + validation (records the rendering model — MOSS, Higgs or Breeze; Breeze design renders store a NULL voice link), claim/submit/fail state transitions, audio & word-timing metadata (`alignment_json`) & deletion, every lookup scoped to a user id
internal/runpod/     the RunPod client — POST /run per engine (MOSS, Higgs, Breeze — the last with `response_delivery` pinned to base64 and clone/design/direction modes), GET /status/{id}, GET /health, output decoded as object-or-aggregate-array (incl. the optional `word_timings` forced-alignment block), permanent-vs-transient errors, payload limits (Higgs: 4 references / 4 MiB each / 6 MiB total; Breeze: 4 MiB per clip / 6 MiB total — decoded, matching each worker's cap)
